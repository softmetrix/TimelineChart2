...
var TimelineChart = (function() {
    ...
    function _createSwimlanes() {
      for(i = 0; i < _data.length; i++) {
        ...
        html += '<div class="col-xs-2 swimlane-title" style="background-color: '+_data[i].color+';height:'+ _data[i].height +'px">';
        html += _data[i].title;
        html += '</div>';
    ...
    function _populateSwimlanes() {
      ...
      for(i = 0; i < _data.length; i++) {
        ...
        for(j = 0; j < _data[i].single.length; j++) {
          ...
          if(cls.class) {
              var html = '<div class="swimlane-item swimlane-item-single swimlane-item-class-' + _data[i].single[j].class + ' ' + cls.class + '" \
              ...
              var html = '<div class="swimlane-item swimlane-item-single swimlane-item-class-' + _data[i].single[j].class + '" \
                           style="left:'+positionPixel+'px; \
                                  top:'+top+'px;">';
              html += '</div>';
              html = $(html);
        ...
        for(j = 0; j < _data[i].range.length; j++) {
          ...
          var html = '<div class="swimlane-item swimlane-item-range swimlane-item-class-' + _data[i].range[j].class + '" \
                           id="tlc_item_rng_'+ id +'" \
    ...
    function _createLegend() {
      ...
      Object.keys(_classes).forEach(function(key,index) {
        ...
        if(!_classes[key].parentClass) {
          ...
          html += "<div class='timeline-legend-title'>" + (_classes[key].title ? _classes[key].title : key) + "</div>";
          html += "</div>";
        }
      });
      html += "</div>";
